...
func (s *SessionApi) GetSignIn(ctx *gin.Context) {
...
//用户进行登录
func (s *SessionApi) PostSignIn(ctx *gin.Context) {
	var (
		res      = gin.H{}
		remember bool
	)
	...
	_ = s.OperationSession(ctx, api.SESSION_KEY, key)
	res["succeed"] = true
...
func (s *SessionApi) LogoutGet(c *gin.Context) {
	session := sessions.Default(c)
	session.Delete(api.SESSION_KEY)
	_ = session.Save()
	c.Redirect(http.StatusSeeOther, "/admin/signin")
}

//修改密码功能
func (s *SessionApi) GetPassword(ctx *gin.Context) {
	ctx.HTML(http.StatusOK, "auth/password.html", gin.H{
		"title": "Wblog | Modify Your Password",
	})
}

func (s *SessionApi) ModifyPassword(ctx *gin.Context) {
...
func (s *SessionApi) UpdatePassword(ctx *gin.Context) {
	s.repository = repositories.NewUserRepository(ctx)
	password := ctx.PostForm("password")
...
//发送邮件or验证码
func (s *SessionApi) SendNotice(ctx *gin.Context) {
	s.repository = repositories.NewUserRepository(ctx)
...
//错误处理
func (s *SessionApi) errorHandler(ctx *gin.Context, err error, path string, h gin.H) {
	_ = seelog.Error(err)
...
//生成重置密码对应的内容
func (s *SessionApi) sendPasswordContext(hash string) (content string) {
